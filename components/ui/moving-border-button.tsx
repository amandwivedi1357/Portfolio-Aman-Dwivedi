...
export const MovingBorder = ({
  ...
  const x = useTransform(
    progress,
    (val) => pathRef.current && getPointFromProgress(pathRef.current, val).x
  );
  const y = useTransform(
    progress,
    (val) => pathRef.current && getPointFromProgress(pathRef.current, val).y
  );
  ...
  return (
    <>
      ...
      >
        <path
          d={`M 0 ${ry ?? 50} C 50 0 50 100 100 ${ry ?? 50} C 50 100 50 0 0 ${
            ry ?? 50
          }`}
          fill="none"
          ref={pathRef}
